...
def split_files(folder, training_fraction=0.7):   
...
def create_labeled_examples_training_testing(folder, seed=0, half_window_side_normalized=0.1):    
    ...
    with open('half_window_side_normalized.cPickle', 'wb') as file_out:
        ...
        image_paths_training, image_paths_testing = split_files(folder, training_fraction=0.7)
    labeled_examples_training = create_labeled_examples(folder,image_paths_training,half_window_side_normalized)
    labeled_examples_testing = create_labeled_examples(folder,image_paths_testing,half_window_side_normalized)
...
def show_examples(labeled_examples, message, number_examples_to_show=15):    
...
def train_and_test_model(folder):    
    labeled_examples_training, labeled_examples_testing = create_labeled_examples_training_testing(folder)    
